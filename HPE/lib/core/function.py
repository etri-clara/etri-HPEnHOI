...
def get_loss_and_uncertainty(batch_idx, config, criterion, output, heatmap, heatmap_target, target_joints, target_joints_vis, wdb):
    uncertainty = None
    uncertainty_map = None

    if config.LOSS.UNCERTAINTY:
        pred_keys, uncertainty, unc_norm_heatmap, output = output
        uncertainty_map = softplus(uncertainty)
        ...
        loss = cal_loss(config, criterion, output, heatmap, heatmap_target, batch_idx, wdb)
...
def train(
    ...
    for batch_idx, (input, target_joints, target_joints_vis, heatmap, heatmap_target, meta) in enumerate(train_loader):
        ...
        if config.MODEL.USE_AMP:
            with autocast():
                output = model(input)
                # TODO 깔끔하게
                output, loss, uncertainty, uncertainty_map = get_loss_and_uncertainty(batch_idx, config, criterion, output, heatmap, heatmap_target, target_joints, target_joints_vis, wdb)
        ...
        if batch_idx % config.PRINT_FREQ == 0:
            ...
            msg += f"Speed[{input.size(0)/batch_time.val:.1f} samples/s] "
            ...
            save_debug_images(config, input, meta, target_joints, pred * 4, output, prefix)
            if wdb:
...
def validate(
    ...
    with torch.no_grad():
        ...
        epoch_start_time = datetime.now()
        for batch_idx, (input, target, target_joints_vis, heatmap, heatmap_target, meta) in enumerate(
            val_loader
        ):
